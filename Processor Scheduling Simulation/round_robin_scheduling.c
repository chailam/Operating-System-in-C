...
int main(int argc, char *argv[])
{
	...
	int j = 0, signal = 1, sizeQ, process, exe_time, k, q;
	...
	while (i < 1024)
	{
		if (j < (num-1))
		{
			...
			else if (signal == 1) //no other process executing
			{
				if (arrival[j] == i)
				{
		...
		sizeQ = sizeQueue(&queue); //size of the queue
		if (sizeQ != 0)
		{
			/* take the process out from queue and execute */
			removeQueue(&queue, &process);
			if (processing[process] > (q-1))
			{
				/* if the remaining processing time of the process > quantum-1, set the executing time to q, print the executing process */
				for (k=0; k<q; k++)
				{
					printf("%d ", process);
				}
			...
			{
				/* if the remaining processing time of the process <= quantum-1, set the executing time to the remaining processing time, print the executing process */
				for (k=0; k<processing[process]; k++)
				{
					printf("%d ", process);
				}
				exe_time = processing[process];
			}
			/* process is executing in the exe_time */
			processing[process] -= exe_time;
			i += exe_time; //increase the time
			signal = 0; //set to executing mode
